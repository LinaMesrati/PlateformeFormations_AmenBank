...
import { GestFService } from 'src/app/Responsable/formateur/service/gestForm';
...
export class AllFormationsComponent implements OnInit {
  ...
  constructor(private service:FormationService,
    private build :FormBuilder, 
     ...
     private serviceFormateur:GestFService){
  ...
  getFormateurs=()=>
  {
    this.serviceFormateur.getFormateurInfo().subscribe((res:any)=>
    {
      this.formateurs=res;
    })
  ...
  getSelectedFormateur(event:any) {
    this.form.get('formateurId')?.setValue(event.target.value)
  ...
  addForm() {
    ...
    console.log('Model to be sent to server:', model);
  
    this.service.createForm(model).subscribe(
      (res) => {
      ...
      (error) => {
        ...
        alert("Error creating formation. Please check the console for details.");
      }
    );
